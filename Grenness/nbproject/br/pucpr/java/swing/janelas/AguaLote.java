...
public class AguaLote extends javax.swing.JFrame{
	 ...
	 * @param mainFrame
	 */
	public AguaLote(JMainFrame mainFrame) {
	...
	private void initComponents() {
		...
		jLabel1.setText("Detec鈬o de ﾁgua");
		...
		variavelK.setText("Detectar Agua por Variacao de Brilho");
		...
		espacoX.setText("Espaﾃｧo X");

		espacoI.setText("Espaﾃｧo I");
		...
		MediaKmeans.setText("Mﾃｩdia");

		CorKmeans.setText("Cor");

		VarianciaKmeans.setText("Variﾃ｢ncia");
	...
	private void CheckTodosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_CheckTodosActionPerformed
		...
		// OPﾃ�ﾃグ TODOS (FORMULAS) SELECIONADO
	...
	private void ProcessarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ProcessarActionPerformed
		...
		try {
			...
			if (!CaminhoImagemOriginal.getText().isEmpty() && !CaminhoSaidaImagem.getText().isEmpty()) {
				...
				//FILE SEPARATOR: CORRESPONDENTE A SEPARAﾃ�ﾃグ DE PASTA DO SISTEMA OPERACIONAL UTILIZADO
				...
				if (variavelK.isSelected()) {
					String k = JOptionPane.showInputDialog(null, "Entre com um valor para maxHorizontal: ");
					double maxHorizontal = Double.parseDouble(k);
					k = JOptionPane.showInputDialog(null, "Entre com um valor para maxVertical: ");
					double maxVertical = Double.parseDouble(k);
					k = JOptionPane.showInputDialog(null, "Entre com um valor para minBrilho: ");
					double minBrilho = Double.parseDouble(k);
					for (File pathvariavel : pathsOriginal) {
						String imagevariavel = pathvariavel.getName();
						//DIVISAO DO NOME DA IMAGEM SEM SUA EXTENSﾃグ PARA COMPARAﾃ�ﾃグ FUTURA
						imagevariavel = imagevariavel.substring(0, (imagevariavel.length() - 4));
						BufferedImage imgvariavel = ImageIO.read(pathvariavel);
						BufferedImage resvariavel, outvariavel = new BufferedImage(imgvariavel.getWidth(), imgvariavel.getHeight(), imgvariavel.getType());
						//CRIA OBJETO DA CLASSE 
						Greenness WA = new Greenness();
						//IMAGEM RESULTANTE DA FORMULA
						resvariavel = WA.WaterDetect(imgvariavel, maxVertical, maxHorizontal, minBrilho);
						Nome = "_WA";
						//SAIDA CONTENDO CAMINHO DA IMAGEM + NOME DA IMAGEM
						String aSaida = pastaSalvar + imagevariavel + Nome + ".png";
						File outputFile = new File(aSaida);
						//CHAMADA METODO KMEANS COMO PARAMETRO A IMAGEM RESULTADO 
//						canvas = new PDIKmeansGray(resvariavel);
//						outvariavel = canvas.getImage();
						//SALVA A IMAGEM
						ImageIO.write(resvariavel, "png", outputFile);
//						ImageIO.write(outvariavel, "png", outputFile);
	...
	private void TodosKmeansActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_TodosKmeansActionPerformed
		...
		// OPﾃ�ﾃグ TODOS (PARAMETROS) SELECIONADO
