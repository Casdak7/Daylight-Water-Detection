...
//            System.out.println(saturation + "Valor Satuaração");
    ...
    public int CalcularDistanciaEuclidiana( Ponto pPonto ){
        int retorno = 0;
        // calcula a raiz quadrada (sqrt) da soma do cálculo da potencia (pow) das três camadas (R, G e B)
        retorno = (int)  Math.sqrt(  
                            Math.pow((pPonto.R-this.R), 2) +
                            Math.pow((pPonto.G-this.G), 2) +
                            Math.pow((pPonto.B-this.B), 2)    
                                    ) ;
        
        return retorno;
    }

    public double CalcularDistanciaEuclidianaD( Ponto pPonto ){
        double retorno = 0;
        // calcula a raiz quadrada (sqrt) da soma do cálculo da potencia (pow) das três camadas (R, G e B)
        retorno =   Math.sqrt(  
                            Math.pow((pPonto.R-this.R), 2) +
    ...
    public void PegarRGBdePonto( Ponto pPonto ){
        this.R = pPonto.R;
        this.G = pPonto.G;
        this.B = pPonto.B;
    }
    ...
    public void PegarCentroideDePontos( PontoCentro pPonto ){
        this.R = (int) pPonto.R;
        this.G = (int) pPonto.G;
        this.B = (int) pPonto.B;
        
        
        this.CentroideR = pPonto.R;
        this.CentroideG = pPonto.G;
        this.CentroideB = pPonto.B;        
        this.CentroideAntR = pPonto.R;
        this.CentroideAntG = pPonto.G;
        this.CentroideAntB = pPonto.B;            
    }
}
