 ...
 */
package pucpr.java.infraBasica.arquivos;
...
import pucpr.java.pdi.RotulacaoBinaria;
...
public class ManipulaArquivo {
    ...
    public void rotulaBinaria() {
        try {
            //abre caminho dos arquivos a serem processados
            File arqImgs[] = abrePasta("Escolha a pasta das imagens a serem Rotuladas (Imagens Binarias)");
            ...
            for (File path : arqImgs) {
                if (path.getName().contains(".jpg") || path.getName().contains(".bmp") || path.getName().contains(".png")) {
                    System.out.println("Processando: " + path.getName());
                    BufferedImage imagem = ImageIO.read(path);
                    RotulacaoBinaria rb = new RotulacaoBinaria();
                    BufferedImage res = rb.rotulacao2(imagem);
                    try {
